...
class WebBundleV01Tests(unittest.TestCase):
    def test_bundle_contains_complete_four_realm_runtime(self):
        bundle = build_bundle()
        self.assertEqual(len(bundle["core_scenes"]), 16)
        self.assertEqual(len(bundle["adaptive_questions"]), 6)
        self.assertEqual(len(bundle["animals"]), 24)
        self.assertEqual(set(bundle["results"]), set(bundle["animals"]))
        self.assertEqual(set(bundle["realms"]), {"Taiga", "Desert", "Rainforest", "Savanna"})

    def test_bundle_contains_four_realm_scoring_runtime(self):
        bundle = build_bundle()
        self.assertEqual(bundle["bundle_version"], "0.6")
        self.assertEqual(len(bundle["boundary_questions"]), 16)
        self.assertEqual(len(bundle["scoring"]["animals"]), 24)
        realms = {
            animal["realm"] for animal in bundle["scoring"]["animals"].values()
        }
        self.assertEqual(realms, {"Taiga", "Desert", "Rainforest", "Savanna"})
        self.assertEqual(
